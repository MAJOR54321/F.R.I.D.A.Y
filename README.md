...
[fish.audio](https://fish.audio) for a specific voice, or leave it blank for that model's default. Audio is
...
[ntfy](https://ntfy.sh) (if configured). It escalates to an **actual phone call** only when presence
...
- **Push (ntfy)**: install the [ntfy app](https://ntfy.sh), subscribe to a long, random topic name, and set
...
1. Fire TV → Settings → My Fire TV → Developer Options → **ADB debugging: ON**. Note its IP → `FIRETV_HOST`.
2. Install Android platform-tools so `adb` is on PATH (or set `ADB_PATH`). She starts the ADB server herself.
3. First command: accept "Allow USB debugging?" on the TV (tick *Always allow*).
